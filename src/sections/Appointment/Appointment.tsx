...
const Appointment = () => {
  ...
  const validateFullName = (value: string) => {
    ...
    if (nameParts.length < 2) {
      setErrors(prev => ({ ...prev, fullName: 'Пожалуйста, введите полное ФИО (минимум 2 слова)' }));
      return false;
    }
  ...
  const validatePhone = (value: string) => {
    ...
    if (phoneDigits.length !== 11) {
      setErrors(prev => ({ ...prev, phone: 'Пожалуйста, введите полный номер телефона (11 цифр)' }));
      return false;
    }
  ...
  const isFormValid = () => {
    ...
    return isFullNameFilled && isPhoneFilled && isEmailFilled && isEmailFormatValid && 
           hasNoErrors && consentPersonal && consentOffer;
  };
          ...
          <div className={styles.infoBlock}>
            ...
            <div className={styles.notice}>
              ...
              <p className={styles.noticeText}>
                онлайн консультация не предполагает выписку справок, рецептов, заключений врача и других медицинских документов.
              </p>
            </div>
          </div>
        </div>
